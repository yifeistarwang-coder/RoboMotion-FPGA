/*传入X,Y(已经扩大了128倍)，传出平方和和扩大2^16倍的角度角*/
...
module cordic_arctan(
	...
	assign theta      = Zn[16] + pai;
	//assign amplitude  = ((Xn[16] >>> 1) + (Xn[16]  >>> 3) +(Xn[16] >>> 4)) >>> 16;  ////幅度，偏大1.64倍，这里做了近似处理 ,然后缩小了2^16
	assign amplitude  = ((Xn[16] >>> 1) + (Xn[16]  >>> 4) + (Xn[16]  >>> 5) + (Xn[16]  >>> 6)) >>> 16;
